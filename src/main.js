const { questions: htmlQuestions } = require('./json/html.json');
const {questions: cssQuestions} = require('./json/css.json');
const {questions: jsQuestions} = require('./json/js.json');
...
$(document).ready(() => {
  ...
  const game = () => {
    ...
    const gameTracker = require('./classes/gameTracker')
    ...
    const getQuestions = () => {
      [html, css, javascript].forEach((type) => {
        if (picked[type]) {
          questionsArray = questionsArray.concat(...allQuestions[type]);
        }
      });
    ...
    const QuestionSet = require('./classes/questionSet');
    let currentQuestion;
    ...
    const setQuestion = () => {
      ...
      if (noQuestionsAvailable) {
        ...
        currentQuestion = new QuestionSet(questionsUsed[0]);
      }
    };
    ...
    const questions = () => {
      ...
      $("#question-type").text(currentQuestion.type.toUpperCase());
      [
        "#question-type",
        "#answer-type",
        "#guess",
    ...
    const setup = () => {
      ...
      const firstTurn = currentGame.turn === 0;
    ...
    const guessPress = () => {
      $("#answer-link").text(currentQuestion.about);
      $("#description").text(currentQuestion.description);
      $("#answer-link").attr("href", currentQuestion.link);
      $("#answer-link").attr("class", "helpful-link");
    ...
    const correct = () => {
      const helpfulLink =
        "<p class='helpful-link'><a target=\"_blank\" href='" +
    ...
    const continuePress = () => {
      ...
      if (suddenDeath) {
        ...
        if (
          (currentGame.turn % 2 === 0 && player1Points > player2Points) ||
          (currentGame.turn % 2 === 0 && player2Points > player1Points)
        ) {
          breakdown();
        } else {
          nextTurn();
